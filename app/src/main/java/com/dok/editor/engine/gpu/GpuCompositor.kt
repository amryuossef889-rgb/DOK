...
class GpuCompositor {
 ...
 fun drawLayer(l:GpuLayer){
  GLES20.glUseProgram(program); val p=GLES20.glGetAttribLocation(program,"aPosition");val t=GLES20.glGetAttribLocation(program,"aTexCoord")
  val m=GLES20.glGetUniformLocation(program,"uMvp");val a=GLES20.glGetUniformLocation(program,"uAlpha");val c=GLES20.glGetUniformLocation(program,"uCrop");val s=GLES20.glGetUniformLocation(program,"uTexture")
  ...
  val vb=java.nio.ByteBuffer.allocateDirect(vertices.size*4).order(java.nio.ByteOrder.nativeOrder()).asFloatBuffer();vb.put(vertices).position(0)
  val tb=java.nio.ByteBuffer.allocateDirect(tex.size*4).order(java.nio.ByteOrder.nativeOrder()).asFloatBuffer();tb.put(tex).position(0)
  GLES20.glEnableVertexAttribArray(p);GLES20.glVertexAttribPointer(p,2,GLES20.GL_FLOAT,false,0,vb);GLES20.glEnableVertexAttribArray(t);GLES20.glVertexAttribPointer(t,2,GLES20.GL_FLOAT,false,0,tb)

...
class MediaPool(private val context: Context) {
    ...
    fun relink(id: String, uri: Uri): MediaAsset? =
        assets[id]?.copy(uri = uri.toString(), isOffline = false)?.also { assets[id] = it }

    fun markOffline(id: String) {
        assets[id]?.let { assets[id] = it.copy(isOffline = true) }
    }
